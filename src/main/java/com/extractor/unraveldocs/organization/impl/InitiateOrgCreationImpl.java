...
public class InitiateOrgCreationImpl {
    ...
    public UnravelDocsResponse<String> initiateCreation(CreateOrganizationRequest request, String userId) {
        log.info("Initiating organization creation for user: {}", userId);
        ...
        // 3. Validate organization name is unique
        ...
        log.info("OTP sent successfully for organization creation. User: {}", userId);

        return new UnravelDocsResponse<>(
                HttpStatus.OK.value(),
                "Success",
                "OTP has been sent to your email. Please verify to complete organization creation.",
                null);
    }

    private void validateUserSubscription(User user) {
        if (user.getSubscription() == null || user.getSubscription().getPlan() == null) {
            throw new ForbiddenException("You need a Premium or Enterprise subscription to create an organization");
        }

        SubscriptionPlans planName = user.getSubscription().getPlan().getName();
        boolean isPremiumOrEnterprise = planName == SubscriptionPlans.PREMIUM_MONTHLY ||
                planName == SubscriptionPlans.PREMIUM_YEARLY ||
                planName == SubscriptionPlans.ENTERPRISE_MONTHLY ||
                planName == SubscriptionPlans.ENTERPRISE_YEARLY;

        if (!isPremiumOrEnterprise) {
            throw new ForbiddenException("Only Premium or Enterprise subscribers can create organizations");
        }
    }
