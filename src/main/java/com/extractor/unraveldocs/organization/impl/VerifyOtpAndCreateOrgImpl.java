...
public class VerifyOtpAndCreateOrgImpl {
    ...
    public UnravelDocsResponse<OrganizationResponse> verifyAndCreate(VerifyOrgOtpRequest request, String userId) {
        log.info("Verifying OTP for organization creation. User: {}", userId);
        ...
        if (otpVerification.getSubscriptionType() == OrganizationSubscriptionType.PREMIUM) {
        ...
        log.info("Organization created successfully: {} ({})", savedOrg.getName(), savedOrg.getId());
    ...
    private void sendConfirmationEmail(User user, Organization org) {
        try {
            ...
            log.debug("Organization confirmation email sent to: {}", user.getEmail());
        } catch (Exception e) {
            log.error("Failed to send organization confirmation email: {}", e.getMessage());
        }
    }
