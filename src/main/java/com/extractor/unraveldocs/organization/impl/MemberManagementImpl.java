...
public class MemberManagementImpl {
    ...
    public UnravelDocsResponse<OrganizationMemberResponse> addMember(String orgId, AddMemberRequest request,
                                                                     String adminId) {
        log.info("Adding member {} to organization {} by {}", request.getUserId(), orgId, adminId);
        ...
        if (!userToAdd.isActive() || !userToAdd.isVerified()) {
            throw new BadRequestException("User must be active and verified to join an organization");
        }
        ...
        log.info("Member {} added to organization {} successfully", request.getUserId(), orgId);
    ...
    public UnravelDocsResponse<Void> removeMember(String orgId, String memberId, String adminId) {
        log.info("Removing member {} from organization {} by {}", memberId, orgId, adminId);
        ...
        log.info("Member {} removed from organization {} successfully", memberId, orgId);
    ...
    public UnravelDocsResponse<Void> removeMembers(String orgId, RemoveMembersRequest request, String adminId) {
        log.info("Batch removing {} members from organization {} by {}", request.getUserIds().size(), orgId, adminId);
        ...
        log.info("{} members removed from organization {} successfully", request.getUserIds().size(), orgId);
    ...
    private void updateUserAdminStatus(User user) {
        // Check if user is admin in any other organization
