...
public class InvitationServiceImpl {
    ...
    public UnravelDocsResponse<String> inviteMember(String orgId, InviteMemberRequest request, String adminId) {
        log.info("Inviting {} to team {} by {}", sanitizer.sanitizeLogging(request.getEmail()), sanitizer.sanitizeLogging(orgId), sanitizer.sanitizeLogging(adminId));
        ...
        if (org.isClosed()) {
            throw new ForbiddenException("Cannot invite members to a closed team");
        }

        // 2. Verify this is an Enterprise team
        ...
        log.info("Invitation sent to {} for team {}", sanitizer.sanitizeLogging(request.getEmail()), sanitizer.sanitizeLogging(orgId));
    ...
    public UnravelDocsResponse<OrganizationMemberResponse> acceptInvitation(String token, String userId) {
        log.info("Accepting invitation with token for user {}", sanitizer.sanitizeLogging(userId));
        ...
        log.info("User {} accepted invitation and joined team {}", sanitizer.sanitizeLogging(userId), sanitizer.sanitizeLogging(org.getId()));
    ...
    private void sendInvitationEmail(String email, Organization org, User invitedBy, String invitationLink) {
        try {
            ...
            log.debug("Invitation email sent to: {}", sanitizer.sanitizeLogging(email));
        } catch (Exception e) {
            log.error("Failed to send invitation email to {}: {}", sanitizer.sanitizeLogging(email), e.getMessage());
        }
    }
